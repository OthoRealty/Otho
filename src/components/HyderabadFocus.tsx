        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 bg-white border border-[#DDD9D0] p-8 flex flex-col justify-between shadow-fine">
            <div>
              ...
              {/* Infrastructure Ground-Truth Audit */}
              {selectedCorridor.infraStatus && selectedCorridor.infraStatus.length > 0 && (
                <div className="mt-6 pt-6 border-t border-[#DDD9D0]">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#111111]">
                      INFRASTRUCTURE GROUND-TRUTH AUDIT
                    </h4>
                    <span className="text-[10px] text-[#888888] uppercase tracking-wider font-mono">
                      SANCTION & FUNDING STATUS
                    </span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {selectedCorridor.infraStatus.map((item, idx) => {
                      const badgeStyles: Record<string, string> = {
                        CONFIRMED: 'bg-emerald-50 text-emerald-800 border-emerald-200',
                        LIKELY: 'bg-amber-50 text-amber-800 border-amber-200',
                        UNFUNDED: 'bg-rose-50 text-rose-800 border-rose-200',
                        UNCERTAIN: 'bg-slate-100 text-slate-700 border-slate-200',
                      };
                      return (
                        <div key={idx} className="p-2.5 bg-[#FBF9F5] border border-[#DDD9D0]/70 flex flex-col justify-between">
                          <div className="flex items-center justify-between gap-1 mb-1">
                            <span className="text-xs font-semibold text-[#111111] leading-tight">
                              {item.name}
                            </span>
                            <span className={`text-[9px] px-1.5 py-0.5 border font-mono font-bold tracking-wider uppercase ${badgeStyles[item.status] || badgeStyles.UNCERTAIN}`}>
                              {item.status}
                            </span>
                          </div>
                          <span className="text-[11px] text-[#666666] leading-snug">
                            {item.detail}
                          </span>
                        </div>
                      );
